...
# Needed to see some solutions to discover a flat list was a handy way to look at this problem.
# Initially tried json parsing and iteration but ran into issues

def convert_to_flat_list(line):
    depth = 0
    num = list()
    for character in line:
        ...
        if character.isdigit():
            num.append([int(character),depth])
    return num
...
def parse_input(input_file):
    ...
    with open(input_file, 'r') as input:
        ...
        while line:
            snailfishno = convert_to_flat_list(line)
            number_lines.append(snailfishno)
            line = input.readline().rstrip()
    return number_lines
